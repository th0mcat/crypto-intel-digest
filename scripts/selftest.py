...
triage scoring, and digest formatting. No network, no DB. Run with:
...
from app.notifier import MATRIX_LIMIT, _chunk  # noqa: E402
...
# --- digest chunking (uses Matrix limit = 16 384 chars) ---
under_limit = _chunk("x" * MATRIX_LIMIT)
check("message at exactly MATRIX_LIMIT stays one chunk", len(under_limit) == 1)
one_long_line = _chunk("y" * (MATRIX_LIMIT + 1000))
check("line over MATRIX_LIMIT splits", len(one_long_line) >= 2)
check("split pieces stay under MATRIX_LIMIT", all(len(c) <= MATRIX_LIMIT for c in one_long_line))
